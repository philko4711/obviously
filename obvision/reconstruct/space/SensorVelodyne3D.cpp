...
void SensorVelodyne3D::returnAngles(double xCoord, double yCoord, double zCoord,
  ...
  *azimAngle = phi;
  ...
  double r = sqrt(xCoord * xCoord + yCoord * yCoord + zCoord * zCoord);

  double theta = acos(yCoord / r);
  if (zCoord > 0)
    theta = -theta;
...
void SensorVelodyne3D::returnAngles(double xCoord, double yCoord, double zCoord,
  ...
  // Inclination
  double theta =
      0.0; // careful love, this is the angle between z-axis and x-y-plane as
  ...
  if (theta >
      deg2rad(90.0)) // translate theta into inclination angle "aperture angle"
                     // from -15° to +15°
  {
  ...
  *azimAngle = atan2(zCoord, xCoord) - M_PI;
  ...
  // if (*azimAngle < 0) {
  //   *azimAngle +=
  //       2.0 * M_PI; // express angles positively in 3rd and 4th quadrant
  // }
}
...
void SensorVelodyne3D::returnRayIndex(double azimAngle, double inclAngle,
  ...
  *azimIndex = static_cast<unsigned>(round(azimAngle / _azimRes));

  // assignment will always be the same for VLP16 - inclination resolution is
  // fixed 2° and nbr of rays is 16
  double mapInclination =
      inclAngle +
      deg2rad(15.0); // map inclination angles (-15° -> +15°) up to positive
  // range 0° - 30° --> TO DO change so this also works for E32
  *inclIndex = static_cast<unsigned>(round(mapInclination / _inclRes));
}
...
void SensorVelodyne3D::backProject(obvious::Matrix *M, int *indices,
                                   obvious::Matrix *T) {
